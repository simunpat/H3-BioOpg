...
public static class UsersSeeder
{
    ...
    {
        // Idempotent seeding: add each user only if not already present (by Id or Email)
        static async Task EnsureUserAsync(AppDbContext db, User u, CancellationToken ct)
        {
            var exists = await db.Users.AsNoTracking()
                .AnyAsync(x => x.Id == u.Id || x.Email == u.Email, ct);
            if (!exists)
            {
                db.Users.Add(u);
            }
        ...
        // Requested user: simun@tec.dk (admin)
        await EnsureUserAsync(db, new User
        {
            Id = Guid.Parse("3b85e8d7-ea4f-4b9a-9695-aea6bc917025"),
            Email = "simun@tec.dk",
            IsAdmin = true,
            PasswordHash = "3806df69721480f0b03c13430eaec13499dfbd81bb9323fa8ca2e04820df7a19",
            PasswordSalt = "948218aa-7a52-4c50-93dd-0ebb2805566f"
        }, ct);
